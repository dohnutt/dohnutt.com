...
class FLCalloutModule extends FLBuilderModule {
    ...
    if ( 'clickable_area' === $this->settings->cta_type ) {
      $classname .= ' fl-callout--link';
    }
   ...
   * @method get_element
   */
  public function get_element( $position = 'open' ) {

    $element = ( 'close' === $position ) ? '</' : '<';

    $element .= ( 'clickable_area' === $this->settings->cta_type ) ? 'a' : 'div';

    if ( 'open' === $position ) {

      if ( 'clickable_area' === $this->settings->cta_type ) {
        $element .= ' href="' . $this->settings->link . '"';
      }

      $element .= ' class="' . $this->get_classname() . '"';
	...
	public function render_title() {
		...
		if ( 'clickable_area' !== $this->settings->cta_type && !empty($this->settings->link) ) {
    ...
    if ( 'clickable_area' !== $this->settings->cta_type && !empty($this->settings->link) ) {
      $settings['link'] = $this->settings->link;
			$settings['link_target'] = $this->settings->link_target;
    }
    ...
    if ( 'clickable_area' !== $this->settings->cta_type && !empty($this->settings->link) ) {
      $settings['link_url_target'] = $this->settings->link_target;
			$settings['link_nofollow'] = $this->settings->link_nofollow;
			$settings['link_type'] = 'url';
			$settings['link_url'] = $this->settings->link;
    }
			...
			'title_structure'   => array(
				...
				'fields' => array(
					'title_color'      => array(
						...
						'preview'     => array(
							'type'     => 'css',
							'selector' => '.fl-callout-title, .fl-callout-title-text',
							'property' => 'color',
						),
					),
			...
			'title_structure'   => array(
				...
				'fields' => array(
					'title_color'      => array(
						...
						'preview'     => array(
							'type'     => 'css',
							'selector' => '.fl-callout:hover .fl-callout-title, .fl-callout .fl-callout-title-text, .fl-callout .fl-callout-title-text:hover',
							'property' => 'color',
						),
					),
				),
	...
	'cta'     => array(
		...
		'sections' => array(
			...
			'cta'        => array(
              ...
              'clickable_area' => __( 'Clickable Area', 'fl-builder' ),
						),
						'toggle'  => array(
							'none'   => array(),
							'link'   => array(
              ...
              /*
              'clickable_area' => array(
                'tabs' => array( 'style_hover' ),
              ),
              */
